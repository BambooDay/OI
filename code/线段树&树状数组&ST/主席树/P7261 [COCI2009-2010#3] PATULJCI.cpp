...
int main(){
	...
	while(m--){
		...
		cin >> l >> r;
		if((r-l+1)&1) {
			int tmp = kth(rt[l-1],rt[r],1,c,(r-l+2)/2);
			if(query(rt[l-1],rt[r],1,c,tmp)>(r-l+1)/2)
				printf("yes %d\n",tmp);
			else puts("no");
		} else {
			int tmp = kth(rt[l-1],rt[r],1,c,(r-l+1)/2),
				tnp = kth(rt[l-1],rt[r],1,c,(r-l+3)/2);
			if(tmp==tnp && query(rt[l-1],rt[r],1,c,tmp) > (r-l+1)/2) 
				printf("yes %d\n",tmp);
			else printf("no\n");
		}		
	}
